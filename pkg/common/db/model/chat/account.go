...
func (o *Account) FindAllUserID(ctx context.Context) ([]string, error) {
	var userIDs []string
	return userIDs, errs.Wrap(o.db.WithContext(ctx).Model(&chat.Account{}).Select("`user_id`").Find(&userIDs).Error)
}
