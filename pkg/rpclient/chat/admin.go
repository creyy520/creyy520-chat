...
func (o *AdminClient) FindUserBlockInfo(ctx context.Context, userIDs []string) (map[string]*admin.BlockInfo, error) {
	blockUserMap := make(map[string]*admin.BlockInfo)
	if len(userIDs) > 0 {
		resp, err := o.client.FindUserBlockInfo(ctx, &admin.FindUserBlockInfoReq{UserIDs: userIDs})
		if err != nil {
			return nil, err
		}
		for i, block := range resp.Blocks {
			blockUserMap[block.UserID] = resp.Blocks[i]
		}
	}
	return blockUserMap, nil
}
